...
(function () {
  ...
  const accionesAplicacion = Object.freeze({
    ...
    readNotification:'marcarNotificacionLeida', notificationSilence:'configurarSilencioNotificaciones', pendingNotices:'avisosPendientes', assignmentAlerts:'listarAvisosAsignacion', respondAssignmentAlert:'responderAvisoAsignacion', resendAssignmentAlert:'reenviarAvisoAsignacion', readAlert:'marcarAlertaLeida', heartbeat:'actualizarConexion', realtimeSummary:'resumenTiempoReal', connectionsOnline:'resumenConexionesAdministrador', saveConnectionTracking:'guardarSeguimientoConexionUsuario',
  ...
  function cargarConexionEmpresa() {
    try {
      ...
      // Formato canónico local 4.3.42. Se leen aliases antiguos solo para migrar
...
(function(){
  ...
  const grupos=[
    ['GENERAL',[
      ...
      ['geoLocal','◉','Geo Planificador de Locales','geo-local.html','GEO_LOCAL'],
      ['mallaTurnos','▦','Malla de Turnos','malla-turnos.html','JORNADA']
    ]],
    ['GESTIÓN',[
    ...
    ['ADMINISTRACIÓN',[
      ...
      ['geofence','⌖','Geocerca','geocerca.html','GEOCERCA'],
      ['geoLocalAdmin','◆','Geo Planificador Admin','geo-local-admin.html','GEO_LOCAL_ADMIN'],
      ['mapaGeoLocal','⌖','Mapa Geo Local','mapa-geo-local.html','MAPA_GEO_LOCAL'],
      ['kpiGeoLocales','◌','KPI Geo Locales','kpi-geo-locales.html','KPI_GEO_LOCALES'],
  ...
  const modulosPaginaIndependiente=new Set(['geoLocal','geoLocalAdmin','mapaGeoLocal','kpiGeoLocales','mallaTurnos']);
  ...
  let oscuro=false;try{localStorage.setItem('flotas_tema','light');}catch(_){}
  ...
  function permitido(modulo){
    ...
    // R8.2.72: la campanita es básica para todo usuario autenticado.
    // El servidor decide qué avisos personales/globales puede ver.
    if(['NOTIFICACIONES','ALERTAS'].includes(modulo))return true;
    ...
    const sysadmin=rol==='ROL-SYSADMIN'||rol==='SYSADMIN';
    const administrador=rol==='ROL-ADMIN'||rol==='ADMINISTRADOR';
    const gerencia=rol==='ROL-GERENCIA'||rol==='GERENCIA';
    // Regla estructural 4.3.29: GEOCERCA es siempre visible para SYSADMIN,
    // Administrador y Gerencia, sin depender de una matriz personalizada vieja.
    if(modulo==='GEOCERCA')return sysadmin||administrador||gerencia;
    if(['GEO_LOCAL_ADMIN','MAPA_GEO_LOCAL','KPI_GEO_LOCALES'].includes(modulo))return sysadmin||administrador||gerencia;
    // Regla general SGF: Administrador y Gerencia tienen acceso funcional total
    // a todos los módulos de su empresa. SYSADMIN mantiene además sus validaciones maestras.
    if(administrador||gerencia||sysadmin)return true;
    ...
    if(rol==='ROL-SUPERVISOR-GEO'){
      const perfilSupervisorGeo=new Set(['PANEL_PRINCIPAL','OFICINA_VIRTUAL','RUTAS','CHECKIN','COMBUSTIBLE','DOCUMENTOS','NOTIFICACIONES','ALERTAS','GEO_LOCAL','JORNADA']);
      if(!perfilSupervisorGeo.has(modulo))return false;
    }
    if(rol==='ROL-CONDUCTOR'){
      const perfilConductor=new Set(['PANEL_PRINCIPAL','OFICINA_VIRTUAL','RUTAS','CHECKIN','COMBUSTIBLE','DOCUMENTOS','NOTIFICACIONES','ALERTAS','JORNADA']);
  ...
  let validacionConfiguracionCpanelPendiente=null;
  async function validarConfiguracionCpanelEnVivo(){
    if(!usuario)return false;
    if(validacionConfiguracionCpanelPendiente)return validacionConfiguracionCpanelPendiente;
    validacionConfiguracionCpanelPendiente=(async()=>{
      try{
        api.invalidate({actions:['me'],resources:['users']});
        const resultado=await api.request('me',{cache:false,force:true});
        const fresco=resultado?.user||resultado?.usuario;
        if(!fresco?.ID)return false;
        api.setAuth({...api.getAuth(),user:fresco});
        aplicarUsuario(fresco);
        construirMenu();
        enviarAutenticacionModulo();
        return configuracionCpanelHabilitada(fresco);
      }catch(error){
        console.warn('No fue posible validar Configuración con cPanel',error);
        return false;
      }finally{
        validacionConfiguracionCpanelPendiente=null;
      }
    })();
    return await validacionConfiguracionCpanelPendiente;
  }

  async function abrirModulo(id,{forzar=false,autorizacionConfiguracionRevisada=false}={}){
    const modulo=modulos.get(id)||modulos.get('dashboard');
    if(!usuario)return;
    if(modulo[0]==='settings'&&!autorizacionConfiguracionRevisada){
      cambiarEstado('Validando autorización de cPanel…');
      const autorizada=await validarConfiguracionCpanelEnVivo();
      if(!autorizada){
        cerrarMenu();
        cambiarEstado('Configuración bloqueada por cPanel','advertencia');
        return;
      }
      return abrirModulo(id,{forzar,autorizacionConfiguracionRevisada:true});
    }
    if(!permitidoModulo(modulo))return;
    ...
    if(marcoListo&&marco.contentWindow&&!forzar&&!modulosPaginaIndependiente.has(moduloIframeActual)&&!modulosPaginaIndependiente.has(modulo[0])){
  ...
  function aplicarUsuario(nuevoUsuario){
    ...
    actualizarInterruptorPausaAvisosExternos();
  ...
  function esRolAdministrativoAvisosMenu(){
    const rol=String(usuario?.ROL_ID_CANONICO||usuario?.ROL_ID||usuario?.ROL_NOMBRE||'').trim().toUpperCase();
    return ['ROL-ADMIN','ADMINISTRADOR','ROL-GERENCIA','GERENCIA'].includes(rol);
  }
  function avisosExternosPausadosMenu(){
    return esRolAdministrativoAvisosMenu()
      && String(usuario?.NOTIFICACIONES_SILENCIADAS||'NO').toUpperCase()==='SI';
  }
  function actualizarInterruptorPausaAvisosExternos(){
    const fila=$('#pausaAvisosExternosMenu'),input=$('#interruptorPausaAvisosExternos');
    if(!fila||!input)return;
    const visible=esRolAdministrativoAvisosMenu();
    fila.hidden=!visible;
    input.checked=visible&&avisosExternosPausadosMenu();
    input.disabled=false;
  }
  async function cambiarPausaAvisosExternosMenu(){
    const input=$('#interruptorPausaAvisosExternos');
    if(!input||!esRolAdministrativoAvisosMenu())return;
    const activo=input.checked;
    input.disabled=true;
    try{
      const result=await api.request('notificationSilence',{
        data:{SILENCIADAS:activo?'SI':'NO',MODO:'SOLO_FUERA_APP'}
      });
      const fresco=result?.user||result?.usuario;
      if(fresco){
        usuario={...usuario,...fresco};
        const auth=api.getAuth();
        api.setAuth({...auth,user:usuario});
      }else{
        usuario={...usuario,NOTIFICACIONES_SILENCIADAS:activo?'SI':'NO'};
      }
      input.checked=avisosExternosPausadosMenu();
      cambiarEstado(
        activo
          ?'Avisos externos pausados · bandeja sigue activa'
          :'Avisos externos activados',
        'listo'
      );
    }catch(error){
      input.checked=!activo;
      cambiarEstado('No se pudo cambiar la pausa de avisos','advertencia');
    }finally{
      input.disabled=false;
    }
  }
  ...
  function avisosEmergentesActivosMenu(){return true;}
  function restaurarAvisosEmergentesMenuR8262(){
    try{
      Object.keys(localStorage).filter(k=>k.startsWith('flotas_avisos_emergentes_admin_v1_')).forEach(k=>localStorage.removeItem(k));
    }catch(_){}
  }
  restaurarAvisosEmergentesMenuR8262();
  ...
  function puedeAvisos(modulo){
    if(!usuario)return false;
    return ['NOTIFICACIONES','ALERTAS'].includes(String(modulo||'').toUpperCase());
  }
...
function esAvisoSilencioso(item){return String(item?.CATEGORIA_EMERGENTE||item?.categoria_emergente||'').toUpperCase().startsWith('GEO_LOCAL_SILENCIOSA_')}
  ...
  async function responderAlertaAsignacionMenu(item,respuesta,boton){
    ...
    try{
      const result=await api.request('respondAssignmentAlert',{id:item.ID,data:{NOTIFICACION_ID:item.ID,RESPUESTA:respuesta}});
      ...
      colaAsignacionesMenu=colaAsignacionesMenu.filter(row=>String(row?.ID||'')!==String(item.ID));
      await actualizarAvisos();
      if(result?.VENCIDA===true||result?.vencida===true){
        cambiarEstado(
          String(result?.MOTIVO||result?.motivo||'').toUpperCase()==='RUTA_CANCELADA'
            ?'Ruta cancelada · tarjeta retirada'
            :'Asignación ya no vigente · tarjeta retirada',
          'listo'
        );
      }else cambiarEstado('Asignación aceptada','listo');
  ...
  function sincronizarTarjetaAsignacionMenu(notificacionesServidor){
    const vigentes=new Set(
      (notificacionesServidor||[])
        .filter(esAlertaAsignacion)
        .map(row=>String(row.ID||''))
        .filter(Boolean)
    );

    colaAsignacionesMenu=colaAsignacionesMenu.filter(
      row=>!row?.ID||vigentes.has(String(row.ID))
    );

    const visibleId=String(
      alertaAsignacionVisible?.dataset?.assignmentId||''
    );
    if(visibleId&&!vigentes.has(visibleId)){
      alertaAsignacionVisible?.remove();
      alertaAsignacionVisible=null;
      cambiarEstado('Asignación retirada o cancelada','listo');
      setTimeout(mostrarSiguienteAlertaAsignacionMenu,120);
    }
  }
  ...
  function mostrarSiguienteAlertaAsignacionMenu(){
    if(esRolAdministrativoAvisosMenu())return;
  ...
  function mostrarAlertaAsignacionMenu(item){
    if(esRolAdministrativoAvisosMenu()||!avisosEmergentesActivosMenu()||!item?.ID)return;
  ...
  async function actualizarAvisos(){
    ...
    consultaAvisosPendiente=(async()=>{
      ...
      const puedeNotificaciones=true,puedeAlertas=true;
      const notificacionesNuevas=deduplicarAvisosMenu((pendientes.notifications||pendientes.notificaciones||[]).filter(item=>item.LEIDA!=='SI'),'notificacion');
      const alertasNuevas=deduplicarAvisosMenu((pendientes.alerts||pendientes.alertas||[]).filter(item=>item.LEIDA!=='SI'),'alerta');
      sincronizarTarjetaAsignacionMenu(notificacionesNuevas);
      ...
      if(avisosInicializados){
        [...nuevasAlertas.filter(item=>!esAlertaVelocidad(item)).map(item=>({item,tipo:'alerta'})),...nuevasNotificaciones.filter(item=>!esAlertaAsignacion(item)&&!esAvisoSilencioso(item)).map(item=>({item,tipo:'notificacion'}))]
  ...
  function aplicarTema(){
    ...
    document.body.classList.remove('oscuro','dark');
    document.documentElement.classList.remove('tema-oscuro-inicial');
    document.documentElement.style.colorScheme='light';
    $('#cambiarTemaMenu')?.remove();
    enviar({tipo:'flotas:tema',oscuro:false});
  }
  ...
  async function validarSesion({desdeModulo=false}={}){
    ...
    validacionPendiente=(async()=>{
      ...
      try{
        const detalleVerificador=$('#verificadorSesion span');
        if(detalleVerificador)detalleVerificador.textContent='Validando empresa y conexión real…';

        // Puerta 1: Directorio central BDEMPRESAFLOTA.
        // Puerta 2: salud de la URL real antes de validar la sesión SGF.
        await api.validarEmpresaActivaParaAcceso({forzar:true});

        if(detalleVerificador)detalleVerificador.textContent='Empresa autorizada. Validando sesión…';
        if(auth.user&&!desdeModulo)await esperar(200);
        const resultado=await api.request('me',{cache:false});
        const usuarioSesion=resultado?.user||resultado?.usuario;
        ...
        const detalleVerificador=$('#verificadorSesion span');
        if(detalleVerificador)detalleVerificador.textContent='No fue posible validar empresa/API. Regresando al acceso…';
        cambiarEstado('No fue posible validar empresa o conexión real','advertencia');
        setTimeout(()=>irAcceso('conexion'),1400);
        return false;
      }
  ...
  window.addEventListener('message',event=>{
    ...
    if(data.tipo==='flotas:empresa'){
      ...
      if($('#logoEmpresaMenu')) $('#logoEmpresaMenu').src='efleet-mark-compact.png';
  ...
  const logoSuperiorInicio=$('#efleetTopHomeLogo');
  if(logoSuperiorInicio){
    const irInicio=()=>{
      cerrarMenu();
      cerrarPanelAvisos();
    ...
    logoSuperiorInicio.addEventListener('click',irInicio);
    logoSuperiorInicio.addEventListener('keydown',event=>{
  ...
  $('#interruptorPausaAvisosExternos')?.addEventListener('change',cambiarPausaAvisosExternosMenu);
