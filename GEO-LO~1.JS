...
// 4.4.30: este helper aplica SOLO a asignaciones; registrar un Local nunca depende del Check-in.
function supervisorGeoListoParaAsignarLocal(sup){const m=sup?.MOVILIDAD||{},v=m.VEHICULO||{},c=m.CHECKIN||{};return Boolean((sup?.ASIGNACION_GEO_HABILITADA===true||sup?.PUEDE_RECIBIR_LOCALES===true)||(v.ID&&(c.LISTO_PARA_VISITAS||c.APROBADO)))}
function esErrorAsignacionGeoCheckin(e){const m=String(e?.message||e||'').toUpperCase();return m.includes('GEO_ASIGNACION_LOCAL_REQUIERE_')}
function irCheckinAsignacionGeo(sup){const nombre=sup?.NOMBRE||sup?.CORREO||'el Supervisor',m=sup?.MOVILIDAD||{},v=m.VEHICULO||{},c=m.CHECKIN||{};let motivo=!v.ID?'no tiene vehículo asignado':!(c.LISTO_PARA_VISITAS||c.APROBADO)?'no tiene un Check-in aprobado y vigente':'debe regularizar su Check-in';try{sessionStorage.setItem('efleet_geo_checkin_supervisor',String(sup?.ID||''))}catch(_){}toast(`No se puede asignar el Local: ${nombre} ${motivo}. Abriendo Check-in…`);setTimeout(()=>abrirModuloCheckin(),650)}
...
async function initAdmin(){
  ...
  async function abrirReasignacion({source,date,scope,detailIds=[]}){const supervisors=(state.data?.SUPERVISORES||[]).filter(x=>x.ID!==source);if(!supervisors.length)return toast('No existe otro Supervisor disponible.');const overlay=document.createElement('div');overlay.className='geo-forced-overlay';overlay.innerHTML=`<div class="geo-forced-card">${geoDialogHead('⇄','Reasignación auditada',scope==='VISITAS_PENDIENTES'?'Reasignar visita pendiente':scope==='SEMANA_COMPLETA'?'Reasignar pendientes de la semana':'Reasignar pendientes del día','Seleccione un Supervisor de la misma empresa. El Supervisor destino debe tener vehículo asignado y Check-in aprobado/vigente antes de recibir cualquier Local.')}<div class="geo-dialog-body"><div class="geo-combo-grid"><label class="geo-field"><span>Filtro rápido Supervisor destino</span><input id="reassignSearch" type="search" placeholder="Nombre, correo o ID"></label><label class="geo-field"><span>Supervisor destino</span><select id="reassignSup"></select></label></div><label class="geo-field"><span>Motivo obligatorio</span><textarea id="reassignReason" placeholder="Indique el motivo de la reasignación"></textarea></label></div><div class="buttons"><button class="geo-btn primary" id="reassignConfirm">Confirmar reasignación</button><button class="geo-btn" id="reassignCancel">Cancelar</button></div></div>`;document.body.appendChild(overlay);poblarSelectFiltrable($('#reassignSearch'),$('#reassignSup'),supervisors,x=>x.NOMBRE||x.CORREO||x.ID,{valueFn:x=>x.ID});$('#reassignCancel').onclick=()=>overlay.remove();$('#reassignConfirm').onclick=async()=>{const reason=$('#reassignReason').value.trim(),dest=$('#reassignSup').value;if(!dest)return toast('Seleccione Supervisor destino.');if(reason.length<3)return toast('Ingrese el motivo.');const sup=supervisorGeoPorId(dest);if(sup&&!supervisorGeoListoParaAsignarLocal(sup)){overlay.remove();irCheckinAsignacionGeo(sup);return}try{setLoading(true);await request('geoLocalReasignar',{SUPERVISOR_ORIGEN_ID:source,SUPERVISOR_DESTINO_ID:dest,ALCANCE:scope,FECHA:date,FECHA_INICIO:date,FECHA_FIN:plusDays(date,6),DETALLE_IDS:detailIds,MOTIVO:reason,EMERGENCIA:'SI'});overlay.remove();toast('Reasignación completada y auditada.');await load()}catch(e){if(esErrorAsignacionGeoCheckin(e)){overlay.remove();irCheckinAsignacionGeo(supervisorGeoPorId($('#reassignSup')?.value)||{ID:$('#reassignSup')?.value||'',NOMBRE:'Supervisor'});return}error(e)}finally{setLoading(false)}}}
