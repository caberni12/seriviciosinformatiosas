...
async function navegarDetalle(item,proveedor){if(!item)return;const lat=Number(item.LOCAL_LATITUD??item.LATITUD),lng=Number(item.LOCAL_LONGITUD??item.LONGITUD);if(!Number.isFinite(lat)||!Number.isFinite(lng))return toast('El Local visitado no tiene coordenadas válidas.');if(!isAdmin()&&!state.data?.LISTO_PARA_VISITAS&&item.CHECKIN_EXENTO!=='SI')return toast('Debe completar un Check-in aprobado y vigente para el vehículo asignado antes de iniciar navegación o cualquier visita Geo Local.');try{await registrarNavegacion(item.ID,proveedor);if(proveedor==='WAZE')openWaze(lat,lng);else openGoogleMaps(lat,lng)}catch(e){error(e)}}
...
function supervisorGeoPorId(id){const listas=[state.data?.SUPERVISORES,state.catalog?.SUPERVISORES].filter(Array.isArray);for(const rows of listas){const u=rows.find(x=>String(x?.ID||'')===String(id||''));if(u)return u}return null}
// 4.4.32: registrar y asignar Locales no depende del Check-in; la barrera aplica solo al iniciar actividad Geo.
function supervisorGeoListoParaAsignarLocal(_sup){return true} // 4.4.32: asignar Locales no exige Check-in; solo iniciar visitas.
function esErrorAsignacionGeoCheckin(_e){return false}
function irCheckinAsignacionGeo(sup){const nombre=sup?.NOMBRE||sup?.CORREO||'el Supervisor';try{sessionStorage.setItem('efleet_geo_checkin_supervisor',String(sup?.ID||''))}catch(_){}toast(`${nombre} tiene la planificación asignada, pero no puede iniciar visitas hasta completar un Check-in aprobado y vigente.`);setTimeout(()=>abrirModuloCheckin(),650)}
...
async function initGeoLocal(){
  ...
  <div class="geo-card" style="margin-top:16px"><div class="geo-toolbar"><div><h2 style="margin:0">Planificación de múltiples destinos</h2><small>${checkinOk||admin?'Puntos habilitados según su planificación.':'La planificación sigue visible, pero navegación, llegada e inicio de visita permanecen bloqueados hasta tener vehículo asignado + Check-in aprobado y vigente.'} La planificación permanece disponible hasta las 23:59.</small></div><label class="geo-field"><span>Filtro rápido</span><input id="geoOwnFilter" type="search" placeholder="Local visitado, dirección o estado"></label></div><div class="geo-list">${items.length?items.map((x,i)=>`<article class="geo-stop" data-search="${esc([x.LOCAL_NOMBRE,x.LOCAL_DIRECCION,x.ESTADO,x.LOCAL_CODIGO,x.LOCAL_REFERENCIA].join(' '))}"><div class="geo-order">${i+1}</div><div><div class="title">${esc(x.LOCAL_NOMBRE||'Local visitado')}</div><div class="meta"><span>${esc(x.LOCAL_DIRECCION)}</span><span>Objetivo ${x.DURACION_ESTIMADA_MIN||x.LOCAL_TIEMPO_OBJETIVO_MIN||30} min</span><span>Radio ${x.LOCAL_RADIO_METROS||100} m</span>${badge(x.ESTADO)}</div><div class="meta"><span>Llegada: ${time(x.LLEGADA_EFECTIVA_EN)}</span><span>Traslado: ${x.TRASLADO_DESDE_ANTERIOR_MINUTOS??'—'}${x.TRASLADO_DESDE_ANTERIOR_MINUTOS!=null?' min':''}</span><span>Permanencia: ${x.PERMANENCIA_MINUTOS??'—'}${x.PERMANENCIA_MINUTOS!=null?' min':''}</span><span>Salida: ${time(x.SALIDA_EFECTIVA_EN)}</span><span>Evidencia: ${esc(x.EVIDENCIA_ESTADO||'PENDIENTE')}</span><span><b>Visitas detectadas hoy: ${Number(x.VISITAS_DIA||0)}</b></span></div></div><div class="buttons"><button class="geo-btn" data-waze="${x.ID}" ${!puedeNavegar?'disabled':''}>Waze</button><button class="geo-btn" data-gmaps="${x.ID}" ${!puedeNavegar?'disabled':''}>Google Maps</button><button class="geo-btn ok" data-arrive="${x.ID}" ${!operar||!checkinOk||x.LLEGADA_EFECTIVA_EN?'disabled':''}>He llegado</button><button class="geo-btn dark" data-photo="${x.ID}" ${!operar||!checkinOk||!x.LLEGADA_EFECTIVA_EN||x.ESTADO==='VISITADO'?'disabled':''}>Foto</button><button class="geo-btn primary" data-finish="${x.ID}" ${!operar||!checkinOk||!x.LLEGADA_EFECTIVA_EN||x.SALIDA_EFECTIVA_EN||x.EVIDENCIA_ESTADO!=='COMPLETA'?'disabled':''}>Finalizar visita</button><input type="file" accept="image/*" capture="environment" hidden data-file="${x.ID}"></div></article>`).join(''):empty('No hay Locales visitados planificados para esta fecha.')}</div></div>`;
...
async function initAdmin(){
  ...
  async function abrirReasignacion({source,date,scope,detailIds=[]}){const supervisors=(state.data?.SUPERVISORES||[]).filter(x=>x.ID!==source);if(!supervisors.length)return toast('No existe otro Supervisor disponible.');const overlay=document.createElement('div');overlay.className='geo-forced-overlay';overlay.innerHTML=`<div class="geo-forced-card">${geoDialogHead('⇄','Reasignación auditada',scope==='VISITAS_PENDIENTES'?'Reasignar visita pendiente':scope==='SEMANA_COMPLETA'?'Reasignar pendientes de la semana':'Reasignar pendientes del día','Seleccione un Supervisor de la misma empresa. Los Locales se reasignan de inmediato; si el destino no tiene Check-in vigente, verá la planificación pero no podrá iniciar visitas hasta regularizarlo.')}<div class="geo-dialog-body"><div class="geo-combo-grid"><label class="geo-field"><span>Filtro rápido Supervisor destino</span><input id="reassignSearch" type="search" placeholder="Nombre, correo o ID"></label><label class="geo-field"><span>Supervisor destino</span><select id="reassignSup"></select></label></div><label class="geo-field"><span>Motivo obligatorio</span><textarea id="reassignReason" placeholder="Indique el motivo de la reasignación"></textarea></label></div><div class="buttons"><button class="geo-btn primary" id="reassignConfirm">Confirmar reasignación</button><button class="geo-btn" id="reassignCancel">Cancelar</button></div></div>`;document.body.appendChild(overlay);poblarSelectFiltrable($('#reassignSearch'),$('#reassignSup'),supervisors,x=>x.NOMBRE||x.CORREO||x.ID,{valueFn:x=>x.ID});$('#reassignCancel').onclick=()=>overlay.remove();$('#reassignConfirm').onclick=async()=>{const reason=$('#reassignReason').value.trim(),dest=$('#reassignSup').value;if(!dest)return toast('Seleccione Supervisor destino.');if(reason.length<3)return toast('Ingrese el motivo.');const sup=supervisorGeoPorId(dest);try{setLoading(true);await request('geoLocalReasignar',{SUPERVISOR_ORIGEN_ID:source,SUPERVISOR_DESTINO_ID:dest,ALCANCE:scope,FECHA:date,FECHA_INICIO:date,FECHA_FIN:plusDays(date,6),DETALLE_IDS:detailIds,MOTIVO:reason,EMERGENCIA:'SI'});overlay.remove();toast('Reasignación completada y auditada.');await load()}catch(e){error(e)}finally{setLoading(false)}}}
  async function saveLocal(ev){ev.preventDefault();const form=ev.currentTarget||$('#localForm');try{setLoading(true);if(!form)throw new Error('FORMULARIO_LOCAL_NO_DISPONIBLE');const d=Object.fromEntries(new FormData(form).entries());await request('geoLocalGuardarLocal',d);form.reset();const radius=$('#localRadius');if(radius)radius.value='100';adminMapPreview=[];adminMapCirculos=[];sincronizarMapaAdmin(false);toast('Local guardado en el catálogo correctamente. No se asignó a ningún Supervisor.');await load()}catch(e){error(e)}finally{setLoading(false)}}
