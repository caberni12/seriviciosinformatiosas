...
function irPanelPrincipalDesdeIcono(){
  try{abrirModulo('dashboard',{forzar:true});}catch(_){location.href='main.html';}
}
...
function esAvisoSilencioso(item){return String(item?.CATEGORIA_EMERGENTE||item?.categoria_emergente||'').toUpperCase().startsWith('GEO_LOCAL_SILENCIOSA_')}
  ...
  $('#logoEmpresaMenu')?.addEventListener('click',irPanelPrincipalDesdeIcono);
  $('#nombreEmpresaMenu')?.addEventListener('click',irPanelPrincipalDesdeIcono);
  document.querySelector('.efleet-shell-mark')?.addEventListener('click',irPanelPrincipalDesdeIcono);
