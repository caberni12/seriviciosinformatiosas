...
async function initAdmin(){
  ...
  function renderControl(){
    ...
    const timelineCards=plans.map(p=>{const sup=supMap.get(p.SUPERVISOR_USUARIO_ID)||{},supervisor=sup.NOMBRE||sup.CORREO||p.SUPERVISOR_USUARIO_ID||'Supervisor',ds=details.filter(d=>d.PLANIFICACION_ID===p.ID).sort((a,b)=>Number(a.ORDEN||0)-Number(b.ORDEN||0)),visibles=ds.filter(d=>{if(!qLocal)return true;const l=lm.get(d.LOCAL_ID)||{},v=vm.get(d.ID)||{};return normalizarFiltro([l.NOMBRE,l.DIRECCION,l.REFERENCIA,d.ESTADO,v.OBSERVACIONES].join(' ')).includes(qLocal)});if(qLocal&&!visibles.length)return'';const visitados=ds.filter(d=>String(d.ESTADO||'').toUpperCase()==='VISITADO').length,enVisita=ds.filter(d=>String(d.ESTADO||'').toUpperCase().includes('EN_VIS')).length,llegadas=ds.map(d=>vm.get(d.ID)?.LLEGADA_EFECTIVA_EN).filter(Boolean).sort(),salidas=ds.map(d=>vm.get(d.ID)?.SALIDA_EFECTIVA_EN).filter(Boolean).sort(),totalPerm=ds.reduce((a,d)=>a+Number((d.TIEMPO_TOTAL_VISITAS_MINUTOS??vm.get(d.ID)?.PERMANENCIA_MINUTOS) || 0),0),totalTras=ds.reduce((a,d)=>a+Number(vm.get(d.ID)?.TRASLADO_DESDE_ANTERIOR_MINUTOS||0),0),visitasFisicas=ds.reduce((a,d)=>a+Number(d.VISITAS_DIA||0),0);
      const stages=visibles.map((d,index)=>{const l=lm.get(d.LOCAL_ID)||{},v=vm.get(d.ID)||{},e=em.get(d.ID),llegada=v.LLEGADA_EFECTIVA_EN||'',salida=v.SALIDA_EFECTIVA_EN||'',done=Boolean(salida)&&String(d.ESTADO||'').toUpperCase()==='VISITADO',started=Boolean(llegada),estadoDetalle=String(d.ESTADO||'').toUpperCase(),pending=estadoDetalle==='PENDIENTE',gestionable=!['VISITADO','CANCELADO','CANCELADA'].includes(estadoDetalle),origenLlegada=v.LLEGADA_MANUAL_EN?'Manual':v.LLEGADA_AUTOMATICA_EN?'Geocerca automática':'Pendiente',origenSalida=v.SALIDA_MANUAL_EN?'Manual':v.SALIDA_AUTOMATICA_EN?'Geocerca automática':'Pendiente',cierreFuera=String(v.CIERRE_FUERA_UBICACION||'').toUpperCase()==='SI',cierreDireccion=v.CIERRE_DIRECCION||'',cierreLat=v.CIERRE_LATITUD??v.SALIDA_LATITUD,cierreLng=v.CIERRE_LONGITUD??v.SALIDA_LONGITUD,cierreDist=v.DISTANCIA_CIERRE_METROS??v.DISTANCIA_SALIDA_METROS,cierreFueraRadio=v.DISTANCIA_FUERA_RADIO_METROS;return `<div class="${done?'completed':started?'active':'pending'}"><i>${done?'✓':d.ORDEN||index+1}</i><span><div class="geo-stage-heading"><b>${esc(l.NOMBRE||'Local visitado')}</b>${geoStageStatus(d.ESTADO,started)}</div><small class="geo-stage-address">${esc(l.DIRECCION||'')}</small><div class="geo-stage-times"><span><small>Llegada</small><strong>${esc(time(llegada))}</strong><em>${esc(dateOnly(llegada))}</em></span><span><small>Permanencia total</small><strong>${d.TIEMPO_TOTAL_VISITAS_TEXTO||((d.TIEMPO_TOTAL_VISITAS_MINUTOS??v.PERMANENCIA_MINUTOS)!=null?`${d.TIEMPO_TOTAL_VISITAS_MINUTOS??v.PERMANENCIA_MINUTOS} min`:(started?'En curso':'—'))}</strong><em>Sumatoria dentro del perímetro</em></span><span><small>Traslado</small><strong>${v.TRASLADO_DESDE_ANTERIOR_MINUTOS??'—'}${v.TRASLADO_DESDE_ANTERIOR_MINUTOS!=null?' min':''}</strong><em>Desde punto anterior</em></span><span><small>Salida</small><strong>${esc(time(salida))}</strong><em>${esc(origenSalida)}</em></span></div>${cierreFuera?`<div class="geo-close-outside"><b>Finalización fuera de ubicación planificada</b><span>${esc(cierreDireccion||'Dirección no resuelta')}</span><span>Coordenadas de término: ${Number(cierreLat).toFixed(6)}, ${Number(cierreLng).toFixed(6)}</span><span>Distancia al Local: ${Number(cierreDist||0).toFixed(0)} m${cierreFueraRadio!=null?` · fuera del radio: ${Number(cierreFueraRadio).toFixed(0)} m`:''}</span></div>`:''}${e?`<button class="geo-btn mini" data-evidence="${esc(e.ID)}">Ver foto</button>`:`<em class="geo-evidence-label">Evidencia: ${esc(v.EVIDENCIA_ESTADO||'PENDIENTE')}</em>`}<em class="geo-evidence-label"><b>Visitas detectadas hoy: ${Number(d.VISITAS_DIA||0)}</b></em>${geoAdminLineaTiempoDetalle(d)}${pending?`<button class="geo-btn mini" data-reassign-detail="${esc(d.ID)}" data-plan="${esc(p.ID)}" data-source="${esc(p.SUPERVISOR_USUARIO_ID)}" data-date="${esc(p.FECHA)}">Reasignar visita</button>`:''}${gestionable?`<button class="geo-btn mini warn" data-cancel-detail="${esc(d.ID)}">Cancelar punto</button><button class="geo-btn mini danger" data-delete-detail="${esc(d.ID)}">Eliminar punto</button>`:''}${v.OBSERVACIONES||d.MOTIVO_NO_VISITA?`<em>${esc(v.OBSERVACIONES||d.MOTIVO_NO_VISITA)}</em>`:''}</span></div>`}).join('');
      return `<article class="geo-trace-card"><header class="geo-trace-head"><div><div class="geo-eyebrow">Tarjeta de trazabilidad · ${esc(p.FECHA||'')}</div><h3>${esc(supervisor)}</h3>${movilidadHtml(sup)}<p>Línea de tiempo tipo Asignación de Ruta: llegada, traslado, permanencia y salida por cada Local visitado.</p></div>${badge(p.ESTADO)}</header><div class="geo-trace-kpis"><span><small>Asignados</small><b>${ds.length}</b></span><span><small>Visitados</small><b>${visitados}</b></span><span><small>En visita</small><b>${enVisita}</b></span><span><small>Primera llegada</small><b>${esc(time(llegadas[0]))}</b></span><span><small>Última salida</small><b>${esc(time(salidas.at(-1)))}</b></span><span><small>Permanencia total</small><b>${totalPerm} min</b></span><span><small>Traslado total</small><b>${totalTras} min</b></span><span><small>Visitas detectadas</small><b>${visitasFisicas}</b></span></div><div class="geo-trace-actions"><button class="geo-btn" data-notify-sup="${esc(p.SUPERVISOR_USUARIO_ID)}" data-plan="${esc(p.ID)}">Enviar alerta</button><button class="geo-btn" data-reassign-day="${esc(p.ID)}" data-source="${esc(p.SUPERVISOR_USUARIO_ID)}" data-date="${esc(p.FECHA)}">Reasignar pendientes del día</button><button class="geo-btn dark" data-reassign-week="${esc(p.ID)}" data-source="${esc(p.SUPERVISOR_USUARIO_ID)}" data-date="${esc(p.FECHA)}">Reasignar semana</button></div><div class="geo-route-timeline" aria-label="Trazabilidad de Locales visitados de ${esc(supervisor)}">${stages||empty('Sin Locales visitados que coincidan con el filtro.')}</div></article>`}).filter(Boolean).join('');
    $('#adminTable').innerHTML=summary+`<h3 class="geo-trace-title">Trazabilidad por Supervisor</h3><p class="geo-trace-subtitle">Cada tarjeta se construye automáticamente desde el primer Local visitado hasta el último.</p>${timelineCards||empty('Sin trazabilidad para el filtro seleccionado.')}`;reconstruirPuntosProgramados(adminMapProgramados.length===0);
